...
//number of integrand components for Cuba
//Since we are integrating the gradient wrt jet momenta, should be
//max_num_jets
#define GRADIENT_COMPONENTS 10 //DS in case of 10 jet events!
//number of variated transfer function evaluations
#define VARIATED_COMPONENTS 110
#define CUBA_COMPONENTS (GRADIENT_COMPONENTS + VARIATED_COMPONENTS + 1)
#define NUM_PDF_INTERP 1000
...
class Integrand {
 ...
 public:
  ...
  void set_hypo(const Hypothesis::Hypothesis);
  ...
  const std::array<double, MEM::Object::NUM_TF_PARS> get_tf_global(TFType::TFType type, int etabin) const;

  double Eval(const double *x, vector<double>& derivs, vector<double>& variations, int only_this_perm = -1);
 ...
 private:
  ...
  void do_integration(unsigned int npar, double *xL, double *xU,
                      double &prob, double &prob2, double &err2, double &chi2, vector<double>& grad, vector<double>& variated);
  ...
  */
  double probability(const double *x, const std::size_t n_perm, vector<double>& derivs, vector<double>& variations);
  ...
  */
  ...
  // evaluate TF
  double transfer(const PS &ps, const vector<int> &perm, int &accept, vector<double>& derivs, vector<double>& variations) const;
  ...
  double pdf_interp_x[NUM_PDF_INTERP];
  double pdf_interp_y[NUM_PDF_INTERP];
  double pdf_interp_z[NUM_PDF_INTERP][NUM_PDF_INTERP];
  double pdf_interp_q_sig[NUM_PDF_INTERP];
  
  gsl_interp * pdf_interpolator_q_sig;
  gsl_interp2d * pdf_interpolator_q_bkg;
  gsl_interp_accel *pdf_interpolator_accel_x;
  gsl_interp_accel *pdf_interpolator_accel_y;
  ...
  // reconstruction
  // efficiency
  const std::map<std::pair<TFType::TFType, int>, std::array<double, MEM::Object::NUM_TF_PARS>> tf_map;
